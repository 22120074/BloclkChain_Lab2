...
contract MyUSDEngine is Ownable {
    ...
    function calculateCollateralValue(address user) public view returns (uint256) {
        // Lấy số lượng ETH người dùng đã nạp
        uint256 ethAmount = s_userCollateral[user];

        // Nếu không có ETH, giá trị là 0
        if (ethAmount == 0) return 0;

        // Lấy giá ETH từ Oracle của bạn
        uint256 price = i_oracle.getETHUSDPrice();

        // Tính toán giá trị: (Lượng ETH * Giá ETH) / 1e18
        // Chia cho PRECISION vì cả ethAmount và price đều có 18 chữ số thập phân
